...
from schema_term_tracker import schema_term_tracker
...
class ScrollAnnotateTermWindow(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.schema = schema_term_tracker
        self.ui_schema = {}
        ...
        # create save button
        self.buttonSave = QtWidgets.QPushButton(text="Save term",parent=self)
        ...
        self.formWidgetList[self.formWidgetNameList.index("termName")].on_changed.connect(lambda saveStatus: self.check_term_name_unique("check"))
        ...
        self.setWindowTitle("Annotate Term")
        #self.show()
    ...
    def check_term_name_unique(self, saveStatus):
        ...
        print("self.termNameList: ",self.termNameList)
        currentTermName = self.formWidgetList[self.formWidgetNameList.index("termName")].text()
        currentTermId = self.formWidgetList[self.formWidgetNameList.index("termId")].text()
        print("currentTermName: ",currentTermName)

        if currentTermName == "default-term-name":
            if saveStatus != "save":
                messageText = "<br>You've re-set the term name to the default value of \"default-term-name\". This is the equivalent of NOT naming your term. You must name your term - Please enter a unique term name that is NOT equal to \"default-term-name\". Term names already in use include: <br><br>" + "<br>".join(self.termNameList)
                errorFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(errorFormat.format(messageText))
            
            if saveStatus == "save":
                messageText = "<br>Your term will be saved with the default term name of \"default-term-name\". This is the equivalent of NOT assigning a name to your term. You must name your term - You can do so by editing your term: Edit your term by navigating to the \"Term Tracker\" tab >> \"Add Term\" sub-tab, clicking on the \"Edit existing term\" push-button, opening the form for this term, and editing the Term Name form field."
                errorFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(errorFormat.format(messageText))

        elif currentTermName in self.termNameList:
            ...
            currentTermNameDf = self.termNameDf[self.termNameDf["termName"] == currentTermName]
            currentAssociatedTermId = currentTermNameDf["termId"].tolist()[0]
            print(currentAssociatedTermId)

            if currentTermId != currentAssociatedTermId:

                if saveStatus != "save":
                    messageText = "<br>You've used this term name before, and term name must be unique - Please enter a unique term name. Term names already in use include: <br><br>" + "<br>".join(self.termNameList)
                    errorFormat = '<span style="color:red;">{}</span>'
                    self.userMessageBox.append(errorFormat.format(messageText))
            ...
            if saveStatus != "save": 
                messageText = "<br>Your term name is unique! <br><br>" 
    ...
    def get_id(self):
        ...
        if self.saveFolderPath:
            ...
            self.form.widget.state = {
                    "termIdNumber": self.annotationIdNum
                }
            ...
            messageText = messageText + "<br><br>Your new term annotation file will be saved in your working DSC Data Package directory as: " + self.saveFilePath + "<br><br>"
            self.userMessageBox.append(messageText)
            #self.userMessageBox.moveCursor(QTextCursor.End)

            
            self.form.widget.state = {
                "termId": self.annotation_id
            }
    ...
    def save_annotation(self):
        ...
        if not os.path.exists(os.path.join(self.workingDataPkgDir,"term-tracker.csv")):
            messageText = "<br>There is no Term Tracker file in your working Data Package Directory; Your working Data Package Directory must contain a Term Tracker file to proceed with saving. If you need to change your working Data Package Directory or create a new one, head to the \"Data Package\" tab >> \"Create or Continue Data Package\" sub-tab to set a new working Data Package Directory or create a new one. Then return here and try saving again.<br><br>"
        ...
        try: 
            with open(os.path.join(self.workingDataPkgDir,"term-tracker.csv"),'r+') as f:
                ...
                messageText = "<br>The Term Tracker file in your working Data Package Directory is open in another application, and must be closed to proceed with saving; You can leave this form window open while you check to see if the Term Tracker file is open in Excel or similar application. Make sure the file is closed, then return here and try saving again. <br><br>"
                
                saveFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(saveFormat.format(messageText))
                return

        annotation = deepcopy(self.form.widget.state)
        ...
        if not dsc_pkg_utils.validateFormData(self=self,formData=annotation):
            return
        ...
        self.check_term_name_unique("save") # this checks if experimentName is unique, if not sets self.uniqueExpNameOnSave to False, also outputs informative message if exp name is not unique or if it is left/set at the default value of default-experiment-name
        if not self.uniqueTermNameOnSave:
            return
        ...
        if self.mode == "edit":
            ...
            messageText = "<br>In preparation for saving your edited term annotation file, your original termannotation file has been archived at:<br>" + self.saveAnnotationFilePath + "<br><br>"
        ...
        if annotation["termId"] != self.annotation_id:
            
            self.annotation_id = annotation["termId"]
        ...
        if os.path.isfile(self.saveFilePath):
            messageText = "A term annotation file for a term with id " + self.annotation_id + " already exists at " + self.saveFilePath + "<br><br>You may want to do one or both of: 1) Use the View/Edit tab to view your term tracker file(s) and check which term IDs you've already used and added to your tracker, 2) Use File Explorer to navigate to your working Data Package Directory and check which term IDs you've already used (i.e. for which you've already created term annotation files - these files will be called \'term-trk-term-{a number}.txt\'. While you perform these checks, your term tracker form will remain open unless you explicitly close it. You can come back to it, change your term ID, and hit the save button again to save with a term ID that is not already in use. If you meant to edit an existing term annotation file, please use the \"Edit an existing term\" functionality on the \"Add term\" sub-tab.<br><br>" 
            errorFormat = '<span style="color:red;">{}</span>'
            self.userMessageBox.append(errorFormat.format(messageText))
            return
            ...
            print(dumps(annotation, indent=4), file=f)
            f.close()
            ...
            messageText = "<br>Your term was successfully written at: " + self.saveFilePath + "<br><br> Starting to add your term to the Term Tracker now! See below for updates: <br>"
            ...
            self.add_annotation() # add experiment file to experiment tracker
    ...
    def add_annotation(self):
        ...
        if ifileName:
            ...
            if Path(self.workingDataPkgDir) != Path(ifileNameCheckDir).parent:
                messageText = "<br>You selected a term txt file that is not in your working Data Package Directory; You must select a term txt file that is in your working Data Package Directory to proceed. If you need to change your working Data Package Directory, head to the \"Data Package\" tab >> \"Create or Continue Data Package\" sub-tab to set a new working Data Package Directory. <br><br>"
                saveFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(saveFormat.format(messageText))
                return
            ...
            for filename in ifileName:
                ...
                annotation_id = "term-" + IdNumStr
                print("term-id: ", annotation_id)
                ...
                out = validate_against_jsonschema(data, schema_term_tracker)
                print(out["valid"])
                print(out["errors"])
                ...
                if not out["valid"]:
                    ...
                    messageText = "The following term file is NOT valid and will not be added to the Term Tracker file: " + filename + "\n\n\n" + "Validation errors are as follows: " + "\n\n\n" + printErrAll + "\n\n\n"
                ...
                # if valid, continue:
                    ...
                    messageText = "The following term file is valid: " + filename
                    self.userMessageBox.append(messageText)
            ...
            messageText = "<br>You have not selected any term files to add to the term tracker. Please select at least one term file to add."
            errorFormat = '<span style="color:red;">{}</span>'
            self.userMessageBox.append(errorFormat.format(messageText))
            return
        ...
        if not validFiles:
            messageText = "The contents of the Term file(s): " + "\n\n\n" + ', '.join(invalidFiles) + "\n\n\n" + "cannot be added to the Term Tracker file because they did not pass validation. Please review the validation errors for the file(s) printed above." + "Exiting \"Add Term\" function now." 
            self.userMessageBox.append(messageText)
            return
        ...
        # get tracker path
        trackerPath = os.path.join(dscDirPath,"term-tracker.csv")
        ...
        if trackerPath:
            ...
            if os.path.isfile(trackerPath):
                ...
                all_df.sort_values(by = ["termIdNumber"], inplace=True)
                ...
                if invalidFiles:
                    messageText = "The contents of the Term file(s): <br><br>" + ', '.join(invalidFiles) + "<br><br>cannot be added to Term Tracker file because they did not pass validation. Please review the validation errors printed above." 
                    errorFormat = '<span style="color:red;">{}</span>'
                    self.userMessageBox.append(errorFormat.format(messageText))
            
                messageText = "The contents of the Term file(s): <br><br>" + ', '.join(validFiles) + "<br><br>were added as term(s) to the Term Tracker file: <br><br>" + output_path
                errorFormat = '<span style="color:green;">{}</span>'
                self.userMessageBox.append(errorFormat.format(messageText))
        
            else:
                messageText = "There is not a valid term tracker file in the current working Data Package Directory."
                errorFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(errorFormat.format(messageText))
                return
    ...
    def load_file(self):
        ...
        if self.mode == "edit":
            textBit = "edit"
            textButton = "\"Edit an existing term\""
        elif self.mode == "add-based-on":
            textBit = "base a new term upon"
            textButton = "\"Add a new term based on an existing term\""

        
        ifileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select the Term txt file you want to " + textBit,
            self.saveFolderPath, "Text (*.txt)")

        if not ifileName: 
            messageText = "<br>You have not selected a file to " + textBit + ". Close this form now. If you still want to " + textBit + " an existing term, Navigate to the \"Term Tracker\" tab >> \"Add Term\" sub-tab and click the " + textButton + " push-button."
            saveFormat = '<span style="color:red;">{}</span>'
            self.userMessageBox.append(saveFormat.format(messageText)) 
        ...
        #     self.userMessageBox.append(saveFormat.format(messageText))
            ...
            if not Path(ifileName).stem.startswith("term-trk-term-"):
                messageText = "<br>The file you selected may not be a term txt file - a term txt file will have a name that starts with \"term-trk-term-\" followed by a number which is that term's ID number. You must select a term txt file that is in your working Data Package Directory to proceed. <br><br> To proceed, close this form and return to the main DSC Data Packaging Tool window."
                saveFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(saveFormat.format(messageText))
                return
            ...
            if Path(ifileName) not in self.filesCheckList: 
                #messageText = "My filename: "+ ifileName + "<br>The file you selected is not up to date with the current schema - You may not " + textBit + " a file that is not up to date with the current schema. Current files that are up to date and may be edited now are as follows: <br><br>" + "<br>".join(self.filesCheckList) + "<br><br>If you still want to " + textBit + " an existing resource that is up to date, Navigate to the \"Resource Tracker\" tab >> \"Add Resource\" sub-tab and click the " + textButton + " push-button. Then select a file that is up to date.<br><br>To proceed, close this form and return to the main DSC Data Packaging Tool window."
                messageText = "<br>The file you selected is not up to date with the current schema - You may not " + textBit + " a file that is not up to date with the current schema. Current files that are up to date and may be edited now are as follows: <br><br>" + "<br>".join(stringFilesCheckList) + "<br><br>If you still want to " + textBit + " an existing file that is up to date, Navigate to the \"Term Tracker\" tab >> \"Add Term\" sub-tab and click the " + textButton + " push-button. Then select a file that is up to date.<br><br>To proceed, close this form and return to the main DSC Data Packaging Tool window."
                saveFormat = '<span style="color:red;">{}</span>'
                self.userMessageBox.append(saveFormat.format(messageText))
                return
            ...
            if self.mode == "add-based-on":
                based_on_annotation_id = data["termId"]
            ...
            if self.mode == "edit":         
                ...
                self.annotation_id = data["termId"]
                self.annotationIdNum = int(self.annotation_id.split("-")[1])
                self.annotationFileName = 'term-trk-'+ self.annotation_id + '.txt'
            ...
            if self.mode == "add-based-on":
                self.get_id()
                messageText = "<br>Your new term has been initialized based on information you entered for " + based_on_annotation_id + "<br><br>"
                saveFormat = '<span style="color:blue;">{}</span>'
                self.userMessageBox.append(saveFormat.format(messageText))

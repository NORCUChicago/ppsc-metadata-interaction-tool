...
from layout_scrollannotatetermwidget import ScrollAnnotateTermWindow
...
class TermTrkAddWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, workingDataPkgDirDisplay):
        ...
        self.buttonAnnotate = QtWidgets.QPushButton(text="Add a new term",parent=self)
        self.buttonAnnotate.clicked.connect(self.annotate)

        self.buttonEdit = QtWidgets.QPushButton(text="Edit an existing term",parent=self)
        self.buttonEdit.clicked.connect(self.edit)

        self.buttonAddBasedOn = QtWidgets.QPushButton(text="Add a new term based on an existing term",parent=self)
    ...
    def annotate(self,checked):
        ...
        if self.w is None:
            self.w = ScrollAnnotateTermWindow(workingDataPkgDirDisplay=self.workingDataPkgDirDisplay,workingDataPkgDir=self.workingDataPkgDir,mode="add")
            self.w.show()
    ...
    def edit(self,checked):
        ...
        if self.w is None:
            ...
            self.w = ScrollAnnotateTermWindow(workingDataPkgDirDisplay=self.workingDataPkgDirDisplay, workingDataPkgDir=self.workingDataPkgDir, filesCheckList=filesCheckList, mode="edit")
            self.w.show()
            self.w.load_file()
    ...
    def annotate_based_on(self,checked):
        ...
        if self.w is None:
            #self.w.editState = True
            self.w = ScrollAnnotateTermWindow(workingDataPkgDirDisplay=self.workingDataPkgDirDisplay, workingDataPkgDir=self.workingDataPkgDir, filesCheckList=filesCheckList, mode="add-based-on")
            self.w.show()
            self.w.load_file()
